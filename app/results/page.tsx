...
import { calculateIQWeighted, calculateIQFull, getIQLabel, getPercentile, type AnswerRecord } from "@/lib/iq-calculator";
    ...
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: "block", margin: "0 auto" }}>
      <defs>
        <linearGradient id="bellFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stopColor="#5B4FFF" stopOpacity="0.18" />
          <stop offset="100%" stopColor="#5B4FFF" stopOpacity="0.02" />
        </linearGradient>
        <linearGradient id="bellShade" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stopColor="#5B4FFF" stopOpacity="0.35" />
          <stop offset="100%" stopColor="#5B4FFF" stopOpacity="0.06" />
        </linearGradient>
      </defs>
      <line x1={padL} y1={padT + plotH} x2={padL + plotW} y2={padT + plotH} stroke="rgba(91,79,255,0.15)" strokeWidth={1} />
      <path d={areaD} fill="url(#bellFill)" />
      {shadeD && <path d={shadeD} fill="url(#bellShade)" opacity={0.7} />}
      <path d={curveD} stroke="#5B4FFF" strokeWidth={2} fill="none" strokeLinejoin="round"
        style={{ filter: "drop-shadow(0 0 4px rgba(91,79,255,0.6))" }} />
      <line x1={userX} y1={padT} x2={userX} y2={padT + plotH} stroke="#5B4FFF" strokeWidth={1.5} strokeDasharray="4,3" opacity={0.7} />
      <circle cx={userX} cy={userY} r={12} fill="rgba(91,79,255,0.2)" />
      <circle cx={userX} cy={userY} r={5}  fill="#5B4FFF" style={{ filter: "drop-shadow(0 0 6px rgba(91,79,255,0.9))" }} />
      <text x={userX} y={padT - 6} textAnchor="middle" fontSize={10} fill="#5B4FFF" fontWeight="700"
        style={{ filter: "drop-shadow(0 0 8px rgba(91,79,255,0.7))" }}>{iq}</text>
      ...
      <text x={padL + plotW}    y={H - 8} textAnchor="middle" fontSize={9} fill="#8AABCC">148</text>
      <line x1={padL + plotW/2} y1={padT + plotH - 4} x2={padL + plotW/2} y2={padT + plotH + 4}
        stroke="rgba(91,79,255,0.3)" strokeWidth={1} />
    </svg>
  );
}
  ...
  return (
    <svg width={180} height={180} viewBox="0 0 180 180">
      {[0.25,0.5,0.75,1].map((p,i)=>(
        <polygon key={i} points={grid(p)} fill="none" stroke="rgba(91,79,255,0.15)" strokeWidth="0.6"/>
      ))}
      {angles.map((a,i)=>(
        <line key={i} x1={cx} y1={cy} x2={cx+R*Math.cos(a)} y2={cy+R*Math.sin(a)} stroke="rgba(91,79,255,0.1)" strokeWidth="0.5"/>
      ))}
      <polygon points={poly} fill="rgba(91,79,255,0.18)" stroke="#5B4FFF" strokeWidth="2"/>
      {pts.map((p,i)=>(
...
function LeaderboardPopup({ iq, onClose }: { iq: number; onClose: () => void }) {
  ...
  const blue  = "#5B4FFF";
  const blue2 = "rgba(91,79,255,0.16)";
  const dim   = "#8AABCC";
  return (
    <div style={{ position:"fixed",inset:0,zIndex:50,background:"rgba(5,10,20,0.82)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div className="animate-fade-up" style={{ maxWidth:380,width:"100%",background:"#080E1A",border:`1px solid ${blue2}`,borderRadius:10,padding:"28px 24px",textAlign:"center",boxShadow:"0 0 60px rgba(91,79,255,0.18)" }}>
          ...
          <>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:18 }}>
              ...
              <div style={{ textAlign:"left" }}>
                <p style={{ fontSize:16,fontWeight:600,color:"#E8E8F0" }}>{country.name}</p>
              </div>
            </div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:20 }}>
              <div style={{ background:"rgba(91,79,255,0.07)",border:`1px solid ${blue2}`,borderRadius:6,padding:"12px 8px" }}>
              ...
              <div style={{ background:"rgba(91,79,255,0.07)",border:`1px solid ${blue2}`,borderRadius:6,padding:"12px 8px" }}>
                ...
                <p style={{ fontSize:22,fontWeight:300,color:iq>=country.avgIQ?"#00D87A":"#FF9F1C" }}>{iq}</p>
              </div>
            </div>
...
async function generateShareImage(iq: number, percentile: number, label: string): Promise<string> {
  ...
  ctx.lineWidth = 0.9;
  conns.forEach(([a,b]) => {
    ctx.strokeStyle = `rgba(91,79,255,${0.08 + Math.random()*0.06})`;
  ...
  nodes.forEach(([x,y],i) => {
    ...
    ctx.fillStyle = i%3===0?"rgba(79,142,255,0.75)":i%3===1?"rgba(0,245,212,0.55)":"rgba(167,139,250,0.55)";
    ctx.fill();
  });

  // ── Central radial glow ──────────────────────────────────────────────────
  const glow = ctx.createRadialGradient(W/2, 480, 0, W/2, 480, 420);
  glow.addColorStop(0, "rgba(91,79,255,0.30)");
  glow.addColorStop(0.45, "rgba(91,79,255,0.10)");
  ...
  [-3,-2,-1,0,1,2,3].forEach((i) => {
    ...
    ctx.fillStyle = Math.abs(i)===0?"#5B4FFF":Math.abs(i)<=1?"rgba(91,79,255,0.6)":"rgba(91,79,255,0.25)";
    ctx.fill();
  });
  ...
  // Glow pass
  ctx.save();
  ctx.shadowColor = "rgba(91,79,255,0.7)"; ctx.shadowBlur = 80;
  const numGrad = ctx.createLinearGradient(W/2-220, 0, W/2+220, 0);
  numGrad.addColorStop(0, "#7C6FFF"); numGrad.addColorStop(0.5, "#5B4FFF"); numGrad.addColorStop(1, "#00F5D4");
  ...
  sep.addColorStop(0,"transparent"); sep.addColorStop(0.5,"rgba(91,79,255,0.7)"); sep.addColorStop(1,"transparent");
  ...
  lblGrad.addColorStop(0,"#5B4FFF"); lblGrad.addColorStop(1,"#00F5D4");
  ...
  // ── Bottom divider ───────────────────────────────────────────────────────
  ctx.strokeStyle = "rgba(91,79,255,0.22)"; ctx.lineWidth = 1;
  ...
  ctx.textAlign = "left";
  ctx.fillStyle = "#EFEFF7"; ctx.fillText("Real", logoStart, 920);
  const iqGrad = ctx.createLinearGradient(logoStart+rW, 900, logoStart+rW+iW, 940);
  iqGrad.addColorStop(0,"#5B4FFF"); iqGrad.addColorStop(1,"#00F5D4");
  ctx.fillStyle = iqGrad; ctx.fillText("IQ", logoStart+rW, 920);
  ctx.fillStyle = "#EFEFF7"; ctx.fillText("Test", logoStart+rW+iW, 920);
...
function ShareModal({ img, iq, percentile, onClose }: {
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:200,background:"rgba(3,5,15,0.94)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div style={{ maxWidth:520,width:"100%",background:"rgba(5,14,40,0.98)",border:`1px solid ${bord}`,borderRadius:18,overflow:"hidden",boxShadow:"0 0 100px rgba(91,79,255,0.2),0 24px 80px rgba(0,0,0,0.6)" }}>
        ...
        <div style={{ padding:"20px 24px 18px",borderBottom:`1px solid ${bord}`,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <div>
            ...
            <h3 style={{ fontSize:19,fontWeight:800,color:"#EFEFF7",letterSpacing:"-0.02em",margin:0 }}>Show your IQ to the world</h3>
          </div>
          <button onClick={onClose} style={{ background:"rgba(91,79,255,0.08)",border:`1px solid ${bord}`,borderRadius:8,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:dim,fontSize:16,flexShrink:0 }}>
        ...
        <div style={{ padding:"20px 24px 24px" }}>
          {/* Image preview */}
          <div style={{ borderRadius:10,overflow:"hidden",border:`1px solid ${bord}`,marginBottom:20,boxShadow:"0 0 40px rgba(91,79,255,0.15)" }}>
          ...
          <button
            ...
            style={{ width:"100%",marginTop:12,padding:"10px 16px",background:"rgba(91,79,255,0.06)",border:`1px solid ${bord}`,borderRadius:8,cursor:"pointer",fontSize:11,color:dim,display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}
          >
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const rawRecords = localStorage.getItem("iq_records");
    const catScoresLS: number[] = JSON.parse(localStorage.getItem("iq_catScores") || "[0,0,0,0,0,0]");
    const catTotalsLS: number[] = JSON.parse(localStorage.getItem("iq_catTotals") || "[0,0,0,0,0,0]");
    let final: number;
    if (rawRecords) {
      const records: AnswerRecord[] = JSON.parse(rawRecords);
      final = calculateIQFull(records, catScoresLS, catTotalsLS);
    } else {
      ...
      final = Math.max(78, Math.min(148, calculateIQWeighted(weighted, maxPoss, minPoss)));
    }
  ...
  return (
    <div style={{ minHeight:"100dvh", background:"#03050F", color:"#E8E8F0" }}>
      <style>{`
      ...
      <div style={{ maxWidth:640,margin:"0 auto",padding:"48px 20px",textAlign:"center" }}>
        ...
        <div style={{
          ...
          <div style={{
            position: "absolute",
            inset: "-20px",
            background: "radial-gradient(ellipse, rgba(91,79,255,0.35) 0%, transparent 70%)",
          ...
          <div className="animate-fade-up" style={{
            ...
            background: "linear-gradient(135deg, #5B4FFF 0%, #00F5D4 40%, #A78BFA 70%, #5B4FFF 100%)",
            backgroundSize: "300% 300%",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
            backgroundClip: "text",
            filter: "drop-shadow(0 0 30px rgba(91,79,255,0.5))",
        ...
        <div className="animate-fade-up" style={{ animationDelay:"220ms",marginTop:16,marginBottom:28 }}>
          <span style={{ display:"inline-block",fontSize:11,letterSpacing:"0.14em",textTransform:"uppercase",padding:"7px 20px",border:`1px solid ${blue}`,color:blue,boxShadow:"0 0 16px rgba(91,79,255,0.3)" }}>{label}</span>
          {iq > 0 && (
            <div style={{
              marginTop: 8,
          ...
          <div className="animate-fade-up" style={{ animationDelay:"250ms",marginBottom:24 }}>
            <button
              ...
              style={{
                ...
                padding:"13px 28px",
                background:"linear-gradient(135deg,rgba(91,79,255,0.18),rgba(0,245,212,0.12))",
                border:"1px solid rgba(91,79,255,0.55)",
                borderRadius:10,cursor:"pointer",
                fontSize:14,fontWeight:700,color:"#EFEFF7",
                boxShadow:"0 0 28px rgba(91,79,255,0.22)",
                transition:"transform 150ms,box-shadow 150ms",
              }}
              onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.transform="translateY(-2px)";(e.currentTarget as HTMLButtonElement).style.boxShadow="0 6px 32px rgba(91,79,255,0.4)";}}
              onMouseLeave={e=>{(e.currentTarget as HTMLButtonElement).style.transform="translateY(0)";(e.currentTarget as HTMLButtonElement).style.boxShadow="0 0 28px rgba(91,79,255,0.22)";}}
            >
              {shareLoading ? (
                <>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#00F5D4" strokeWidth="2" className="animate-spin">
                    <circle cx="12" cy="12" r="10" strokeOpacity="0.25"/><path d="M12 2a10 10 0 0110 10" stroke="#00F5D4"/>
                  </svg>
                  Generating card…
                </>
              ) : (
                <>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#00F5D4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {personalPct !== null && userCountry && (
          <div className="animate-fade-up" style={{ animationDelay:"400ms",marginBottom:24,padding:"12px 18px",background:"rgba(91,79,255,0.06)",border:`1px solid rgba(91,79,255,0.3)`,borderRadius:8,textAlign:"center" }}>
            <p style={{ fontSize:13,lineHeight:1.6,color:"#E8E8F0" }}>
              You scored higher than{" "}
              <span style={{ background:"linear-gradient(90deg,#5B4FFF,#00F5D4)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",fontWeight:700 }}>
                {personalPct}%
              </span>
        ...
        <div className="animate-fade-up" style={{ animationDelay:"450ms",marginBottom:20,background:"rgba(5,18,45,0.75)",border:`1px solid ${blue2}`,backdropFilter:"blur(14px)",borderRadius:8,padding:"24px",filter:"blur(7px)",opacity:0.5,userSelect:"none",pointerEvents:"none",textAlign:"center" }}>
          ...
          <div style={{ height:4,background:blue2,borderRadius:2,overflow:"hidden",marginTop:16,maxWidth:280,margin:"16px auto 0" }}>
            <div style={{ height:"100%",width:"87%",background:"linear-gradient(90deg,#5B4FFF,#00F5D4)",borderRadius:2 }}/>
          </div>
        </div>
        ...
        <div className="animate-fade-up" style={{
          ...
          background:"linear-gradient(135deg,rgba(91,79,255,0.12),rgba(0,245,212,0.06))",
          border:"1px solid rgba(91,79,255,0.45)",
          borderRadius:14,overflow:"hidden",
          boxShadow:"0 0 50px rgba(91,79,255,0.12),0 4px 32px rgba(0,0,0,0.4)",
        }}>
          {/* Top urgency bar */}
          <div style={{ background:"rgba(91,79,255,0.15)",borderBottom:"1px solid rgba(91,79,255,0.3)",padding:"8px 20px",display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#00F5D4" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            <span style={{ fontSize:10,color:"#00F5D4",letterSpacing:"0.14em",fontWeight:600 }}>Your results are saved for 24 hours</span>
          </div>
          ...
          <div style={{ padding:"28px 24px" }}>
            ...
            <p style={{ fontSize:16,fontWeight:700,color:"#E2EEFF",lineHeight:1.5,marginBottom:20,textAlign:"left" }}>
              ...
              <span style={{ background:"linear-gradient(90deg,#5B4FFF,#00F5D4)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text" }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:20,textAlign:"left" }}>
              <div>
                ...
                <span style={{ fontSize:38,fontWeight:800,background:"linear-gradient(135deg,#5B4FFF,#00F5D4)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",letterSpacing:"-0.03em" }}>€1.99</span>
              </div>
            ...
            <button
              ...
              style={{
                ...
                background:"linear-gradient(135deg,#5B4FFF 0%,#00C9AE 50%,#00F5D4 100%)",
                border:"none",borderRadius:10,
                fontSize:17,fontWeight:800,color:"#fff",
                cursor:"pointer",
                boxShadow:"0 6px 32px rgba(91,79,255,0.55),0 0 80px rgba(91,79,255,0.2),inset 0 1px 0 rgba(255,255,255,0.15)",
                transition:"transform 150ms,box-shadow 150ms",
                letterSpacing:"-0.02em",
              }}
              onMouseEnter={e=>{(e.target as HTMLButtonElement).style.transform="translateY(-2px)";(e.target as HTMLButtonElement).style.boxShadow="0 10px 40px rgba(91,79,255,0.65),0 0 100px rgba(0,245,212,0.25)";}}
            ...
            <div style={{ display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginTop:16 }}>
              {["Scientifically calibrated","6 cognitive dimensions","Visual pattern recognition"].map((badge,i)=>(
                <span key={i} style={{ fontSize:10,color:"#8AABCC",background:"rgba(91,79,255,0.08)",border:"1px solid rgba(91,79,255,0.22)",padding:"4px 10px",borderRadius:99 }}>
                  <span style={{ color:"#00F5D4",fontWeight:700,marginRight:4 }}>✓</span>{badge}
                </span>
              ))}
            </div>
            ...
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginTop:14 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#00F5D4" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
              <span style={{ fontSize:11,color:"#00F5D4",fontWeight:600 }}>Your detailed cognitive breakdown expires in 24h</span>
            </div>
          </div>
        </div>
